...
const PlayerFullscreen = () => {
    ...
    useEffect(() => {
        ...
        //@ts-ignore
    }, [adminQuizView.playerCurrentAnswer?.data.content, adminQuizView.currentPlayerViewing])

    useEffect(() => {
        if (adminQuizView.adminViewing) {
            if (key == "ArrowRight") {
                adminQuizView.setCurrentPlayerViewing?.((adminQuizView.currentPlayerViewing ?? 0) + 1);
                setKey("")
    ...
    return <div onKeyDown={handleKeyDown} tabIndex={0} ref={mainComponentLayout} className="h-full w-full fixed bg-background z-[999] grid place-items-center">
        <div className="container py-20 w-full h-full flex flex-col justify-between items-center md:w-3/4">
            <div className="flex flex-row w-full">
                ...
                <div className="flex-1 flex flex-row items-center justify-end gap-4">
                    <Trophy />
                    <span className="text-white font-bold text-3xl text-center">{adminQuizView.playerCurrentResult?.score ?? 0}</span>
                </div>
            </div>
